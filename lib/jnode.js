...
var JNode = function() {
	...
	var get_library = function(name, path) {
		if(libraries[name]) {
			return libraries[name];
		} else {
			return (libraries[name] = require(path));
		}
	}
	...
	/**
	 * The folder that contains all of your libraries.
	 * @constant logs_folder
	 */
	self.libraries_folder = "libraries";
	 ...
	 */
	self.session_max_age = 1000 * 60 * 60 * 24 * 31;
	...
	/**
	 * This function should be called before routing anything. Before this function you can change all the settings. When 
	 * this function is done all the settings are used, so chaning them won't do anything anymore.
	...
	self.socket = function() {
		if(socket == null) {
			return (socket = require('./socket/socket')(self));
		}
		else {
			return socket;
		}
	...
	self.library = function(name) {
		return get_library(name, self.root + self.libraries_folder + "/" + name);
	}
	...
	self.assets = function(folder, type) {
		if(folder.startsWith(self.root)) {
			return router.route_folder(folder.substring(folder.lastIndexOf("/") + 1, folder.length), folder, type);
		}
		else {
	...
	self.view = function(url, file) {
		if(file.startsWith(self.root)) {
			return router.route(url, file, "html");
		}
		else {
	...
	/**
	 * This function will load up a new language and add it to the list of languages. With this function you can easily use 
	 * different locales for every country.
	 * @param  {String} language The name of the language (e.g. "en" for english)
	 * @param  {String} file     The path to the language file
	 */
	...
	/**
	 * This function will return the text with the specified path in the specified langauge (If loaded).
	 * @param  {String} language The name of the language (e.g. "en" for english)
	 * @param  {String} path     The path to the text
	...
	/**
	 * This function will add a plugin to the JNode class. This will mean the functions and variables in the plugin parameter 
	 * will be added to the JNode class if there name hasn't been used yet.
	 * @param  {Object} plugin The functions and variables to add to the JNode class
	 * @return {Promise}       Will return a promise
	 */
		...
		return new promise(function(succes, failure) {
			for(var key in plugin) {
				...
				if(self[key] == null) {
					...
					failure("The method " + key + " is already used!");
				}
			}
