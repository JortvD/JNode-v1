module.exports = function() {
	...
	self.start = function(port = 3000, callback) {
		server = app.listen(port, function() {
			self.helper("logger").debug("Started listening on port " + port + ".");
	...
	self.stop = function(callback) {
		server.close(function() {
			self.helper("logger").debug("Stopped listening on port " + server.address().port + ".");
